...
#define ANCHO_VENTANA 930
#define ALTO_VENTANA 930
...
bool colisionBarrera(char mapa[MAX_FILAS][MAX_COLUMNAS], int pos_x, int pos_y, int ancho_sprite, int alto_sprite) {
    ...
    for (int fila = inicio_y; fila <= fin_y; fila++) {
        for (int columna = inicio_x; columna <= fin_x; columna++) {
...
int main() {
    ...
    for (int fila = 0; fila < MAX_FILAS; fila++) {
        if (fgets(mapa[fila], MAX_COLUMNAS + 1, archivo) == NULL) {
    ...
    if (!fondo) {
        fprintf(stderr, "Error al cargar el fondo.\n");
        return -1;
    }
    ...
    while (!salir) {
        ...
        if (event.type == ALLEGRO_EVENT_TIMER) {
            ...
            if (al_key_down(&keyboardState, ALLEGRO_KEY_UP)) {
                ...
                if (pos_x > 0 && !colisionBarrera(mapa, pos_x - TAMANO_CELDA / 2, pos_y, ancho_sprite, alto_sprite)) {
